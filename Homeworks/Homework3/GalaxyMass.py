...
table1 = [] # this is the start of the table so it can be easily calculated
...
filename = "MW_000.txt" # the selected file for this table
halo = ComponentMass(filename,1)  # halo stars calculated
table1.append([halo]) # appends the halo calculations
disk = ComponentMass(filename,2) # the disk stars to be calculated
table1.append([disk])  # append the disk calcualtions
bulge = ComponentMass(filename,3) # the bulge calculations
table1.append([bulge]) # the bulge calculations being appended to the table
LocalGMass = halo + disk + bulge # this is to sum up the local group mass
table1.append([LocalGMass]) # append the mass to the table
fbar = np.round(disk / LocalGMass, 3) # we need calculate the baryon fraction is the total stellar mass over total mass
table1.append([fbar])  # now append this to the table
print(table1) # print the table 